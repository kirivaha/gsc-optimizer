        ...
        $.post(gscOpt.ajax_url, {
            ...
            .done(function (res) {
                if (res.success) {
                    const d = res.data;
                    $result.html(
                        '<p><strong>Тип:</strong> ' + d.paragraph_type + '</p>' +
                        '<p><strong>Знайдений перший абзац (буде переписаний):</strong></p>' +
                        '<textarea readonly style="width:100%;height:80px;font-size:12px;border:1px solid #ccc;padding:6px;">' + d.found_paragraph + '</textarea>' +
                        '<p><strong>Відповідь AI (' + d.ai_response_length + ' символів):</strong></p>' +
                        '<textarea readonly style="width:100%;height:80px;font-size:12px;border:1px solid #2e7d32;padding:6px;">' + d.ai_response + '</textarea>'
                    );
                } else {
                    $result.html('<p style="color:red;">❌ ' + res.data + '</p>');
                }
